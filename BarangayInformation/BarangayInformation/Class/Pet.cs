...

{
    class Pet
    {
        ...
        public int save()
        {
            ...
            query = "INSERT INTO pets SET pet=?n"; //query for database // INSERT INTO tablename SET columname = 'value'
            cmd = new MySqlCommand(query, con);
            cmd.Parameters.AddWithValue("?n", this.pet); //you can use this keyword, or remove the this keyword
        ...
        public int update(int id)
        {
            ...
            con = Connection.con();
            con.Open();
            query = "UPDATE pets SET pet=?n WHERE pet_id = ?id";
            cmd = new MySqlCommand(query, con);
            cmd.Parameters.AddWithValue("?n", this.pet);
        ...
        public void tableData(DataGridView grid, string key)
        {
            con = Connection.con();
            con.Open();
            query = "SELECT * FROM pets WHERE pet LIKE ?key";
            cmd = new MySqlCommand(query, con);
        ...
        public void getData(int id)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                this.pet = Convert.ToString(dt.Rows[0]["pet"]);
            }
        }
